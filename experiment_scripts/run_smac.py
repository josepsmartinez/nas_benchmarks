...
def smac_search(
        b, benchmark_name,
        n_iters, random_fraction, n_trees, max_feval):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--n_trees', default=10, type=int, nargs='?',
        help='number of trees for the random forest')
    parser.add_argument(
        '--random_fraction', default=.33, type=float, nargs='?',
        help='fraction of random configurations')
    parser.add_argument(
        '--max_feval', default=4, type=int, nargs='?',
    ...
    res = smac_search(
        b, args.benchmark,
        args.n_iters, args.random_fraction,
        args.n_trees, args.max_feval)
